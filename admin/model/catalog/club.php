...
class ModelCatalogClub extends PT_Model
{
    ...
    {
        $query = $this->db->query("INSERT INTO " . DB_PREFIX . "club SET  date = '" . $this->db->escape((string)$data['date']) . "',austin_governor_id = '" .(int)$data['austin_governor_id'] . "',club_name = '" . $this->db->escape((string)$data['name']) . "',president = '" . $this->db->escape((string)$data['president']) . "',district_secretary = '" . $this->db->escape((string)$data['secretary']) . "',assistant_governor = '" . $this->db->escape((string)$data['governor']) . "',mobile = '" . $this->db->escape((string)$data['mobile']) . "',email = '" . $this->db->escape((string)$data['email']) . "', password = '" . $this->db->escape(password_hash(html_entity_decode($data['password'], ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "',website = '" . $this->db->escape((string)$data['website']) . "',image = '" . $this->db->escape((string)$data['image']) . "',ip = ':81',status = '" . (isset($data['status']) ? (int)$data['status'] : 0) . "', date_modified = NOW(), date_added = NOW()");
